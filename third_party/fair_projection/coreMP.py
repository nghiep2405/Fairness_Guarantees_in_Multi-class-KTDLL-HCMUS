...
def admm_tf(G,y,rho=2,div = 'kl', max_iter = 1000, eps1=1e-7, eps2=1e-2, eps3=1e-7, verbose=False, progress_every=50):
    '''
    Core Model Projection algorithm. Here:
    n - number of data points
    k - number of constraints
...
#         Gl_tf_slice.assign(tf.linalg.matmul(G_tf[ix[0]:ix[-1]+1,:,:],l_tf))
    ...
    inner_tol = 1e-10
    for ix in range(max_iter):
        if verbose and ((ix + 1) % progress_every == 0 or ix == 0):
            print(f"[FairProjection ADMM] iteration {ix + 1}/{max_iter}", flush=True)
